...
############ context ###################################
device_context = mx.gpu(0); """set device over which the traning will be perfomed. use mx.cpu() for training over CPU cores, however,
training over CPU will be considerably slower (3 hours on an 8 core machine) compared to 7 minutes on my GPU (640 cores).
...
num_round = 20; #number of epochs (for training rounds)
...
train,retrain,applynet = (False,True,False)
# for retraining the network
(start_epoch,step_epochs) = (30,5)
save_round = start_epoch + step_epochs

# testing data
test_img = "C:\\Users\\aliha\Downloads\\fabrice-ali\\deeplearning\\data\\train\\train_images_8bit\\image350.tif";
...
def get_unet():
    ...
    global filtercount, kernel_size, pad_size, drop
...
##### Resize images
#train_x = np.asarray(list(map(lambda x: np.array(imageResize(x)), imagefilenames)))
...
def custom_rmse(label,pred):
    ...
    for tensor in pred:
        ...
        plt.draw()
        plt.pause(0.005)
    return np.sqrt(np.mean((label-matlist)**2))
...
if train:
    ...
    model.save_checkpoint("blobseg_model_soft", num_round)

    destination = directory + "saved_models\\softmax_saves\\iteration " + str(num_round) + "\\";
    if not os.path.exists(destination):
        os.makedirs(destination)
    filesToCopy = [fname for fname in os.listdir() if fname.startswith("blobseg_model_soft")]
...
if retrain:
    os.chdir(directory + "saved_models\\softmax_saves\\iteration " + str(start_epoch))
    model_prefix = "blobseg_model_soft"
...
if applynet:
    ...
    os.chdir(directory + "saved_models\\softmax_saves\\iteration " + str(save_round))
    model_prefix = "blobseg_model_soft"

    testimgdata = np.asarray(imageResize(test_img)).reshape((1,1,width,height))

    symbolicNet, arg_params, aux_params = mx.model.load_checkpoint(model_prefix,save_round) # loading the trained network
